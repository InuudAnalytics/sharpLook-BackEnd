...
const registerSocketHandlers = (io) => {
    io.on("connection", (socket) => {
        ...
        socket.on("sendMessage", async ({ senderId, receiverId, message, tempId, type, mediaUrl, duration }) => {
            ...
            try {
                // Save message to DB
                const saved = await (0, message_service_1.saveMessage)(senderId, receiverId, roomId, message, type, mediaUrl, duration);
        ...
        socket.on("markAsRead", async ({ roomId, userId: readUserId }) => {
            try {
                await (0, message_service_1.markMessagesAsRead)(roomId, readUserId);
...
async function updateUserOnlineStatus(userId, isOnline, lastSeen) {
    try {
...
async function sendCallNotification(receiverId, callerId, callType) {
    try {
        ...
        if (receiver?.fcmToken && caller) {
            ...
            await pushNotifications_service_1.pushNotificationService.sendPushNotification(receiver.fcmToken, `Incoming ${callType} call`, `${callerName} is calling you...`, {
                type: "INCOMING_CALL",
                callerId,
                callType,
            });
